/** @format */

import { useWallet } from "hooks";
import { useEffect } from "react";
import { useSelector } from "react-redux";
import { useLocation, useNavigate } from "react-router-dom";
import BorderImage from "assets/png/header/border.png";
import toast from "react-hot-toast";
import JungleImg from "assets/svg/apeworld/jungle.svg";

const Jungle = () => {
  const { pathname } = useLocation();
  const { address } = useWallet();
  const navigate = useNavigate();
  const { collectionOptions } = useSelector((state: any) => state.collections);

  useEffect(() => {
    if (collectionOptions.length === 0) {
      navigate("/");
    }
  }, [collectionOptions, navigate]);
  return (
    <div className='bg-gradient-to-t from-[#456300] to-[#9fcf4a] text-gray-200 md:px-[10%] tracking-widest lg:px-[13%] p-3 md:pt-40 pt-28 min-h-[100vh] relative'>
      <div className='md:text-5xl text-2xl relative z-20'>
        <div className='flex text-center'>
          <div className={`w-[50%]`} onClick={() => navigate("/casino")}>
            <p
              className={`mb-2 cursor-pointer border-r-2 border-[#762e1550] ${
                pathname === "/jungle" ? "text-gray-100" : "text-gray-800"
              }`}>
              BAR
            </p>
            {pathname === "/jungle" && (
              <img className='w-full' src={BorderImage} alt='borderImg' />
            )}
          </div>
          <div
            className={`w-[50%]`}
            onClick={() => {
              if (address) {
                navigate("/jungle/info");
              } else {
                toast.error("Please connect the Wallet");
              }
            }}>
            <p
              className={` mb-2 cursor-pointer ${
                pathname === "/jungle/info" ? "text-gray-100" : "text-gray-800"
              }`}>
              FIGHTER INFO
            </p>
            {pathname === "/jungle/info" && (
              <img className='w-full' src={BorderImage} alt='borderImg' />
            )}
          </div>
        </div>
        {/* {pathname === "/jungle" ? <Register /> : <Fighter />} */}
      </div>
      <img
        className='absolute bottom-5 right-5 z-10 hidden md:inline opacity-50'
        src={JungleImg}
        alt='pawnShop'
      />
    </div>
  );
};

export default Jungle;
